...
while getopts "hva:c:e:r:u:p:n:" OPTION
do
     case $OPTION in
         h)
             usage
...
if ! curl -f -k -sS -L ${REDIRECT_URL} ${OUT} ${AUTHENTICATION} -v  --location-trusted
then
   echo "unable to download artifact ${REDIRECT_URL}" 1>&2
   exit 1
fi
rpm -Uvh $OUT
